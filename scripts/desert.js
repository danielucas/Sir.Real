...
/***/ function(module, exports, __webpack_require__) {
	...
	var textures = [{
	  file: 'sirs/tumblr_lxx18zlDMq1rnrr8lo1_500.png',
	  name: 'image',
	  align: textureAlign,
	  scale: { x: 5, y: 5 }
	}];
	...
	var curve = function curve(v) {
	  var p = arguments.length <= 1 || arguments[1] === undefined ? 8 : arguments[1];
	...
	haze.gl.createUniform('1i', 'noiseSize', 500);
	
	haze.addEventListener('predraw', function () {
	  haze.gl.createUniform("2f", "noiseR", Math.round(Math.random() * 500), Math.round(Math.random() * 500));
	  haze.gl.createUniform("2f", "noiseG", Math.round(Math.random() * 1), Math.round(Math.random() * 1));
